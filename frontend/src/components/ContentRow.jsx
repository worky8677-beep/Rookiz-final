 ...
 */
export function ContentRow({ title, items, layout = "scroll", badge, filter, onItemClick, showViewAll = true, viewAllLink = "#", className, children }) {
  const GRID_MAX = 4;
  ...
  return (
    <div className={twMerge("w-full flex flex-col gap-7", className)}>
      ...
      {children ? <div className="w-full">{children}</div> : layout === "grid" ? renderGrid() : renderScroll()}
    </div>
  );
}
